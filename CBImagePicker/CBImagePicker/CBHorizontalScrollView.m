...
- (UIImageView *)setupImageViewWithImage:(UIImage *)image {
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(10.f, 5.f, HORZONTAL_IMAGE_CELL_SIZE.width, HORZONTAL_IMAGE_CELL_SIZE.height)];
...
- (void)setImageModel:(CBAssetsModel *)imageModel {
    ...
    if (_imageModel) {
        ...
        imageView.originLeft = 5.f * _assetsArray.count + imageView.sizeWidth * (_assetsArray.count - 1);
        ...
        if (_assetsArray.count == 1) {
            ...
            [self setContentOffset:CGPointMake(imageView.originLeft - (self.sizeWidth - imageView.sizeWidth - 10.f),0) animated:YES];
        }
        ...
        [_assetsArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            imageView.originLeft = 5.f * (idx + 1) + imageView.sizeWidth * idx;
        ...
        if (_assetsArray.count == 1) {
            [self setContentInset:UIEdgeInsetsMake(0, (self.sizeWidth - HORZONTAL_IMAGE_CELL_SIZE.width) / 2 - 10.f, 0, 0)];
...
#pragma CBImageBrowser代理
- (void)imageBrowser:(CBImageBrowser *)browser index:(NSInteger)index {
    [self setContentOffset:CGPointMake(5.f * (index + 1) + HORZONTAL_IMAGE_CELL_SIZE.width * index - (self.sizeWidth - HORZONTAL_IMAGE_CELL_SIZE.width) / 2,0) animated:NO];
}
